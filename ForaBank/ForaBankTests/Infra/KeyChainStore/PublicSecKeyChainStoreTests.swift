...
final class PublicSecKeyChainStoreTests: XCTestCase {
    ...
    func test_save_shouldDeliverLastSavedKey() throws {
        ...
        let firstKey = try anySecKey(.publicKey)
        ...
        let lastKey = try anySecKey(.publicKey)
    ...
    func test_clear_shouldRemoveSavedKey() throws {
        ...
        let key = try anySecKey(.publicKey)
        let expiration = Date()
...
private extension Crypto {
    ...
    static func createPublicSecKey(
        from data: Data
    ) throws -> SecKey {
