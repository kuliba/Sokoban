...
class CheckNumberSBPDefaultVC: UIViewController {
...
//        continueButton.changeEnabled(isEnabled: true)
    }
...
//        }
    ...
    @IBAction func actionNextComplete(_ sender: Any) {
        
        
        activityIndicator?.startAnimation()
        continueButton.isHidden = true
        print("сохраняем значания в defoult")
        
        NetworkManager.shared().getAnywayPaymentFinal(memberId: "codeVerification", amount: codeNumberTextField.text, numberPhone: "", parameters: nil) { (success, anywayInputs, errorMessage) in
            if success ?? false{
                NetworkManager.shared().anywayPaymentMake(code: self.codeNumberTextField.text ?? "") { (success, errorMessage) in
                    if anywayInputs[0]?.result != "ERROR"{
                        self.performSegue(withIdentifier: "completeSBPBank", sender: nil)
                    } else {
...
extension CheckNumberSBPDefaultVC: UITextFieldDelegate {
