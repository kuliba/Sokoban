...
#warning("need a wrapper with timer")
        ...
        case let .productPicker(productID):
            factory.makeProductSelectView(productID, { event(.product($0, $1)) })
        }
    }
}
...
extension AnywayPaymentWidgetView {
    ...
    typealias Factory = AnywayPaymentWidgetViewFactory
}

#Preview {
    
    Group {
        AnywayPaymentWidgetView(widget: .otp(nil), event: { print($0) }, factory: .preview)
        AnywayPaymentWidgetView(widget: .otp(123), event: { print($0) }, factory: .preview)
        AnywayPaymentWidgetView(widget: .otp(123456), event: { print($0) }, factory: .preview)
        
        AnywayPaymentWidgetView(
            widget: .productPicker(.accountID(12345)),
            event: { print($0) },
            factory: .preview
        )
    }
