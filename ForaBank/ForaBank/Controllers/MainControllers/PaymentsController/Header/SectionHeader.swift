...
class SectionHeader: UICollectionReusableView {
    ...
    func configure(text: String, font: UIFont?, textColor: UIColor, expandingIsHidden: Bool, seeAllIsHidden: Bool, onlyCards: Bool, isExpanded: Bool, expandAction: @escaping () -> Void, productSelectorViewModel: OptionSelectorView.ViewModel? = nil) {
        ...
        title.font = font
        title.text = text
        if title.text == "Инвестиции и пенсии"  || title.text == "Услуги и сервисы" {
    ...
    func addProductTypeSelectorView(with viewModel: OptionSelectorView.ViewModel) {
        ...
        let makeCategoryView: MakeCategoryView = { CategoryView(newImplementation: true, isSelected: $0, title: $1) }

        let makeOptionButtonView: MakeOptionButtonView = { .init(viewModel: $0, isSelected: $1, viewFactory: .init(makeCategoryView: makeCategoryView))}
