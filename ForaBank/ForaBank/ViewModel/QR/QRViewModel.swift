...
class QRViewModel: ObservableObject {
    ...
    init(title: String, buttons: [ButtonIconTextView.ViewModel], clouseButton: ButtonSimpleView.ViewModel) {
        self.title = title
    ...
    private func createButtons() -> [ButtonIconTextView.ViewModel] {
        ...
        return [
            ButtonIconTextView.ViewModel(icon: .init(image: .ic24Image, background: .circle), title: .init(text: "Из файла"), orientation: .horizontal, action: { [weak self] in
                self?.action.send(QRViewModelAction.OpenDocument())
            }),
            ButtonIconTextView.ViewModel(icon: .init(image: .ic24ZapOff, background: .circle), title: .init(text: "Фонарик"), orientation: .horizontal, action: { [weak self] in
                self?.action.send(QRViewModelAction.Flashlight())
            }),
            ButtonIconTextView.ViewModel(icon: .init(image: .ic24AlertCircle, background: .circle), title: .init(text: "Инфо"), orientation: .horizontal, action: { [weak self] in
            ...
            .sink { [unowned self] action in
                ...
                case _ as QRViewModelAction.OpenDocument:
                    self.bottomSheet = .init(sheetType: .choiseDocument(.init(buttons: [
                        .init(icon: .init(image: .ic12Clock, background: .circle), title: .init(text: "Из фото"), orientation: .horizontal, action: { [weak self] in
                            self?.bottomSheet = nil
                            self?.model.action.send(ModelAction.Media.GalleryPermission.Request())
                        }),
                        .init(icon: .init(image: .ic12Clock, background: .circle), title: .init(text: "Из Документов"), orientation: .horizontal, action: { [weak self] in
                            print("Document")
                        })
                    ])))
