//
//  LoggingRemoteNanoServiceComposer+composeSerialResultLoadTests.swift
//  VortexTests
//
...
final class LoggingRemoteNanoServiceComposer_composeSerialResultLoadTests: XCTestCase {
    ...
    func test_composeSerial_shouldCallCreateRequestWithNilOnNilSerial() {
        ...
        let sut = makeSUT().sut
        let composed: SerialResultLoad = sut.composeSerialResultLoad(
    ...
    func test_composeSerial_shouldDeliverFailureOnCreateRequestFailure() {
        ...
        expect(
            ...
            with: anyMessage(),
            assert: { XCTAssertNoDiff($0, .failure(.urlRequestCreationFailure)) },
            on: ()
        )
    }
    ...
    func test_composeSerial_shouldDeliverFailureOnHTTPClientFailure() {
        ...
        expect(
            ...
            response: .failure(makeFailure()),
            with: anyMessage(),
            assert: { XCTAssertNoDiff($0, .failure(.performRequestFailure)) },
    ...
    func test_composeSerial_shouldDeliverFailureOnMapResponseFailure() {
        ...
        expect(
            ...
            response: .failure(makeFailure()),
            with: anyMessage(),
            assert: { XCTAssertNoDiff($0, .failure(.mapResponseFailure)) },
    ...
    func test_composeSerial_shouldDeliverFailureOnSameSerial() {
        ...
        expect(
            ...
            with: serial,
            assert: { XCTAssertNoDiff($0, .failure(.noNewDataFailure)) },
            on: httpClientSpy.complete(with: (anyData(), anyHTTPURLResponse()))
        )
    }
    ...
    func test_composeSerial_shouldDeliverStampedOnDifferentSerial() {
        ...
        expect(
            ...
            with: anyMessage(),
            assert: { XCTAssertNoDiff($0, .success(stamped)) },
            on: httpClientSpy.complete(with: (anyData(), anyHTTPURLResponse()))
        )
    }
    ...
    private typealias SerialResultLoad = SUT.SerialResultLoad<Value>
    ...
    private func expect(
        ...
        assert: @escaping (Result<Stamped, SUT.SerialResultLoadError>) -> Void = { _ in },
    ...
    private func expect(
        ...
        composed(serial) {
            ...
            case let .failure(failure):
                switch failure as? SUT.SerialResultLoadError {
                case .none:
                    XCTFail("Expected SerialResultLoadError, but got \($0) instead.", file: file, line: line)
