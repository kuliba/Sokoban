...
public extension CalendarConfig {
    ...
    func startMonth(_ value: Date) -> Self {
    
        MCalendar.startDate = value.start(of: .month)
        return self
    }

    /// Sets the end date of the calendar.
    /// DEFAULT: A date  in 10 years
    func endMonth(_ value: Date) -> Self {
        MCalendar.endDate = value.end(of: .month)
        return self
    }

    /// Sets the first day of the week.
    /// DEFAULT:: Monday
    func firstWeekday(_ value: WeekDay) -> Self {
        
        MCalendar.firstWeekday = value
        return self
    }
...
public extension CalendarConfig {
    ...
    func monthsTopPadding(_ value: CGFloat) -> Self {
        changing(path: \.monthsPadding.top, to: value)
    }

    /// Sets the bottom scroll padding in the view.
    func monthsBottomPadding(_ value: CGFloat) -> Self {
        changing(path: \.monthsPadding.bottom, to: value)
    }

    /// Sets the distance between the month label and the day cell in the view.
    func monthLabelToDaysDistance(_ value: CGFloat) -> Self {
        changing(path: \.monthLabelDaysSpacing, to: value)
    }
    ...
    func daysVerticalSpacing(_ value: CGFloat) -> Self {
        changing(path: \.daysSpacing.vertical, to: value)
    }

    /// Sets the horizontal spacing between day cells in the view.
    func daysHorizontalSpacing(_ value: CGFloat) -> Self {
        changing(path: \.daysSpacing.horizontal, to: value)
    }
}
...
public extension CalendarConfig {
    ...
    func dayView(_ builder: @escaping (Date, Bool, Binding<Date?>?, Binding<MDateRange?>?) -> some DayView) -> Self { 
