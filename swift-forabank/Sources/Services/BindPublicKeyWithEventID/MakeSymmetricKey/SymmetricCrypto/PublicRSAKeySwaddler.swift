...
public final class PublicRSAKeySwaddler<PrivateKey, PublicKey>
where PublicKey: RawRepresentational {
    ...
    public typealias AESEncrypt128bitChunks = (Data) throws -> Data
    ...
    public func swaddle(with otp: OTP) throws -> Data {
        
        let (encryptedOTP, privateRSAKey, publicRSAKey) = try encryptOTP(otp)
        
        try saveKeys(privateRSAKey, publicRSAKey)
        
        let json = try JSONSerialization.data(withJSONObject: [
            "procClientSecretOTP": encryptedOTP.base64EncodedString(),
            "clientPublicKeyRSA": publicRSAKey.rawRepresentation.base64EncodedString()
        ] as [String: Any])
