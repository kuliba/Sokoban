//
//  RemoteServiceDecoratorTests.swift
//
//
...
final class RemoteServiceDecoratorTests: XCTestCase {
    
    func test_init_shouldNotCallCollaborator() {
        
        let (_, decoratorSpy, decorationSpy) = makeSUT()
        
        XCTAssertNoDiff(decoratorSpy.callCount, 0)
        XCTAssertNoDiff(decorationSpy.callCount, 0)
    }
    ...
    func test_process_shouldDecorateDecorateeFailure() {
        ...
        let failure = SUT.ProcessError()
        let (sut, decoratorSpy, decorationSpy) = makeSUT()
    ...
    private func makeSUT(
        ...
        return (sut, decoratorSpy, decorationSpy)
    }
