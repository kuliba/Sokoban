//
//  SerialStampedCachingDecoratorVoidTests.swift
//
//
...
final class SerialStampedCachingDecoratorVoidTests: XCTestCase {
    ...
    func test_load_shouldCallDecorateeWithPayload() {
        ...
        sut.load(serial: anyMessage()) { _ in }
        
        XCTAssertNoDiff(loadSpy.payloads.count, 1)
    }
    ...
    func test_load_shouldNotDeliverFailureOnInstanceDeallocation() {
        ...
        sut?.load(serial: anyMessage()) { _ in exp.fulfill() }
        sut = nil
        loadSpy.complete(with: .failure(anyError()))
    ...
    func test_load_shouldCallCacheOnDifferentLoadedSerial() {
        ...
        sut.load(serial: oldSerial) { _ in }
    ...
    private typealias Payload = Void
