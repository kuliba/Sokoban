//
//  ComposedSymmetricKeyServiceTests.swift
//  
//
...
final class ComposedSymmetricKeyServiceTests: XCTestCase {
    ...
    func test_makeSymmetricKey_shouldNotReceiveSecretRequestResultAfterSUTInstanceHasBeenDeallocated() {
        ...
        var sut: ComposedSymmetricKeyService? = .init(
        ...
        var makeSymmetricKeyResults = [ComposedSymmetricKeyService.Result]()
    ...
    private func makeSUT(
        ...
        let publicServerSessionKeyAPIClient = PublicServerSessionKeyAPIClientSpy()
        let symmetricCrypto = SymmetricCryptoSpy()
        let sut = ComposedSymmetricKeyService(
            secretRequestCrypto: secretRequestCrypto,
            publicServerSessionKeyAPIClient: publicServerSessionKeyAPIClient,
            symmetricCrypto: symmetricCrypto
        )
    ...
    private func expect(
        _ sut: SymmetricKeyService,
        toMakeSymmetricKey expectedResults: [ComposedSymmetricKeyService.Result],
        on action: () -> Void,
        file: StaticString = #file,
        line: UInt = #line
    ) {
